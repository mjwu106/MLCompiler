...
absl::Status AlgebraicSimplifierVisitor::HandleAdd(HloInstruction* add) {
  ...
  // (x / y) + (z / y) => (x + z) / y
  HloInstruction *x, *z, *y;
  if (Match(add, m::Add(m::Divide(m::Op(&x), m::Op(&y)),
                        m::Divide(m::Op(&z), m::Op(&y))))) {
    VLOG(10) << "Applying transformation: (x / y) + (z / y) -> (x + z) / y";
    HloInstruction* new_add = add->parent()->AddInstruction(
        HloInstruction::CreateBinary(add->shape(), HloOpcode::kAdd, x, z));
    HloInstruction* new_div = add->parent()->AddInstruction(
        HloInstruction::CreateBinary(add->shape(), HloOpcode::kDivide, new_add, y));
    return ReplaceWithNewInstruction(add, new_div);
  }
